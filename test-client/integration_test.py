...
class Client:
    ...
    async def connect_raw(self, body):
        """Handshake with an arbitrary JSON body, including shapes the
        server should reject rather than trip over (a bare array)."""
...
async def main():
    # Pairing is mandatory now (the server generates a code when none
    # exists), so the core flow runs with a known one and the enforcement
    # checks below drive it directly.
    TEST_CODE = "246813"
    vita_server_mod.PAIR_CODE = TEST_CODE
    ...
    print("core flow:")
    c = Client(TEST_PORT)
    check(await c.connect({"code": TEST_CODE}) is True,
          "handshake accepted with the pairing code")
    # Pinned to the constant rather than a literal: the version only
    # means anything if both sides move together, and hardcoding it here
    # made the test fail on the bump instead of proving it.
    check(c.server_version == vita_server_mod.PROTOCOL_VERSION,
          "handshake carries the protocol version")
    ...
    check(await bad.connect({"code": "wrong"}) is not True,
          "wrong pairing code rejected")
    nocode = Client(TEST_PORT)
    check(await nocode.connect({}) is not True,
          "handshake without a code rejected")
    weird = Client(TEST_PORT)
    check(await weird.connect_raw([1, 2, 3]) is not True,
          "handshake body that is not an object rejected, not crashed")
    # A non-ASCII code must be REJECTED, not raise: secrets.compare_digest
    # refuses non-ASCII str, which used to break every handshake including
    # the correct one when the operator chose an accented code.
    accented = Client(TEST_PORT)
    check(await accented.connect({"code": "segretò"}) is not True,
          "non-ASCII pairing code rejected without raising")
    good = Client(TEST_PORT)
    check(await good.connect({"code": TEST_CODE}) is True,
          "correct pairing code accepted")
    good.close()
