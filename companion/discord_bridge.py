...
# Per-message ceiling for text sent to the console. A history reply carries
# forty messages in one 64KB frame, and Discord allows far more than this
# per message, so an untrimmed batch could overflow the frame and break the
# connection. Well past what fits on the screen anyway.
MAX_CONTENT_CHARS = 700

# Ceiling on a single downloaded image, applied while reading rather than
# from the declared length (chunked responses declare none).
...
class DiscordBridge:
    ...
    async def on_voice_state_update(self, member, before, after):
        """Someone joined or left a voice channel: the console's connected
        lists are part of the channel list, so tell it to refetch."""
        if before.channel == after.channel:
            return
        guild = getattr(member, "guild", None)
        if self._vita_server and guild:
            await self._vita_server.broadcast(MsgType.VOICE_MEMBERS_STALE,
    ...
    async def join_voice(self, channel_id: int, vita_ip: str) -> bool:
        ...
        log.info("JOIN_VOICE '%s' (bitrate %dkbps, region %s) -> streaming to %s",
                 channel.name, getattr(channel, "bitrate", 0) // 1000,
                 getattr(channel, "rtc_region", None) or "auto", vita_ip)
        from voice import VoiceRelay
        if self._voice is None:
            self._voice = VoiceRelay()
        if self._vita_server:
            self._voice.set_broadcaster(
                lambda names: self._vita_server.broadcast(
                    MsgType.VOICE_SPEAKING, {"names": names}))
        try:
    ...
    async def get_image(self, url: str, size: int = 64) -> bytes | None:
        ...
        try:
            ...
            # Redirects are refused: the allowlist above checks the URL we
            # were given, and following a hop would let an allowed host
            # send the fetch anywhere it liked.
            async with self._http.get(
                url, timeout=aiohttp.ClientTimeout(total=10),
                allow_redirects=False
            ) as resp:
                if resp.status == 200:
                    # Accumulate in chunks with the ceiling applied to the
                    # running total. read(n) is NOT the right tool: it
                    # returns whatever has arrived so far rather than n
                    # bytes, which truncates every real CDN download, and
                    # read() with no argument buffers the whole body before
                    # any size check could stop it.
                    raw = bytearray()
                    async for chunk in resp.content.iter_chunked(65536):
                        ...
                        if len(raw) > IMAGE_MAX_BYTES:
                            break
                    if len(raw) <= IMAGE_MAX_BYTES:
                        data = self._shrink_image(bytes(raw), size)
                    else:
                        log.warning("Image from %s exceeds %d bytes, skipped",
                                    host, IMAGE_MAX_BYTES)
    ...
    def _serialize_message(msg: discord.Message) -> dict:
        ...
        # normal messages. clean_content is the same text with mentions
        # resolved, so <#1234> becomes #general and <@1234> becomes a
        # name: on the Vita the raw form was unreadable noise.
        content = msg.system_content or msg.content or ""
        try:
            resolved = msg.clean_content
            if resolved:
                content = resolved
        except Exception:
            pass
        # Forty of these travel in one frame, and the protocol caps a frame
        # at 64KB. Discord allows 2000 characters (4000 with Nitro), and
        # non-ASCII escapes to six bytes apiece in JSON, so a channel full
        # of long messages could overflow the frame; the console then
        # rejected it and dropped the connection. The console shows a dozen
        # lines at most, so trimming here costs nothing visible.
