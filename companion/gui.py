...
class CompanionGui:
    def __init__(self, root: tk.Tk):
        ...
        # The log is written so it can be pasted into a bug report, so
        # discord.py's INFO output stays out of it: it carries gateway
        # session identifiers and connection details that belong to the
        # account. Its warnings and errors still come through.
    ...
    def start(self):
        # The server owns the pairing code: it loads the environment
        # override or config.json and generates one if neither exists, so
        # every entry point is paired the same way. Assigning it back from
        # here used to clobber the DAWNCORD_PAIR_CODE override.
        self.code_label.config(text=vita_server_mod.PAIR_CODE)
    ...
    def _append(self, line: str):
        ...
        # Trim from the top: the widget grew without limit, so anything
        # that logs in a tight loop (a host hammering the pairing code, for
        # one) would eat memory and freeze the window.
