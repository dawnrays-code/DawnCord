...
# basicConfig configures the ROOT logger, so without this discord.py's own
# INFO chatter lands in the same file: gateway session identifiers, TLS
# fingerprint targets, connection details. The log is meant to be pasteable
# into a bug report, so it must not carry anything that identifies the
# account or its session. Warnings and errors from the library still come
# through, which is what a report actually needs.
