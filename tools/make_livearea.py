...
LiveArea layout notes learned on real hardware:
  - the gate (Start) frame sits in the middle of the screen and covers
...
print("clyde downloaded:", src.size)
...
def font(size: int, bold: bool = True):
    """A TrueType face at the requested size. The bare filenames resolve
    through the system font path on every OS, so this generator produces
    the same assets on Linux and macOS; load_default() is a last resort
    and ignores the size, which silently shrank titles to 10px."""
    if bold:
...
# No credit line down here on purpose. A byline on the LiveArea reads like
# a watermark, and none of Sony's own apps carry one; the README says who
# built this, at length, which is the right place for it.

# Vignette: the corners fall away a touch so the eye lands on the middle,
# where the gate sits. Built from the same ramp trick used elsewhere, one
# band per edge, all of them very faint.
vign = Image.new("RGBA", bg.size, (0, 0, 0, 0))
vd = ImageDraw.Draw(vign)
for i in range(90):
    a = int(52 * (1 - i / 89) ** 2)
    vd.line([(0, i), (839, i)], fill=(8, 6, 24, a))
    vd.line([(0, 499 - i), (839, 499 - i)], fill=(8, 6, 24, a))
for i in range(120):
    a = int(46 * (1 - i / 119) ** 2)
    vd.line([(i, 0), (i, 499)], fill=(8, 6, 24, a))
    vd.line([(839 - i, 0), (839 - i, 499)], fill=(8, 6, 24, a))
...
d.text((120, 56), "Start", font=font(34), fill=(255, 255, 255, 255))
