...
int main(void)
{
    ...
    check(state_parse_guilds(&st,
        ...
        "{\"id\":\"1181234567890123456\",\"name\":\"Test Guild\",\"icon\":null}]}") == 0,
        "guild list parses");
    ...
    check(state_parse_channels(&st,
        ...
        "{\"id\":\"vu:6\",\"name\":\"speaker one, speaker two +2\",\"type\":\"voiceusers\"},"
        "{\"id\":\"6\",\"name\":\"Vocale\",\"type\":\"voice\"}]}") == 0,
    ...
    /* voice speaking push: replaces the set, matched by display name */
    check(state_parse_speaking(&st,
        "{\"names\":[\"speaker one\",\"speaker two\"]}") == 0,
        "speaking list parses");
    check(st.speaking_count == 2 &&
          state_is_speaking(&st, "speaker two") &&
          !state_is_speaking(&st, "nobody"),
        "speaking names matched by display name");
    check(state_parse_speaking(&st, "{\"names\":[]}") == 0 &&
          st.speaking_count == 0 && !state_is_speaking(&st, "speaker two"),
        "empty list clears everyone");
