...
struct vita2d_texture *img_get_icon(const char *url, int size)
{
    char key[IMG_KEY_LEN];
    if (!url || !url[0])
        return NULL;
    if (snprintf(key, sizeof(key), "%s#%dr", url, size) >= (int)sizeof(key))
        return NULL;
    return img_get(key);
}
...
/* Same feathered edge as mask_circle, but the distance is measured past
   the corner centres, giving a rounded square. Baking the shape into the
   texture keeps the edge quality identical to the circular avatars, and
   costs nothing at draw time. */
static void mask_round_rect(vita2d_texture *tex, float radius_frac)
{
    unsigned int w = vita2d_texture_get_width(tex);
    unsigned int h = vita2d_texture_get_height(tex);
    unsigned int stride_bytes = vita2d_texture_get_stride(tex);
...
void img_store(const char *key, const void *jpeg, size_t jpeg_len)
{
    ...
    if (s->tex) {
        ...
        /* Key tells us the shape: a plain URL is an avatar (circle), one
           ending in 'r' is a server icon (squircle), anything else is an
           attachment and stays rectangular. */
        size_t klen = strlen(s->key);
        if (!strchr(s->key, '#'))
            mask_circle(s->tex);
        else if (klen && s->key[klen - 1] == 'r')
            mask_round_rect(s->tex, 0.28f);
    }
