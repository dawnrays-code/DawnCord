...
int state_parse_speaking(app_state *st, const char *json)
{
    cJSON *root = cJSON_Parse(json);
    if (!root)
        return -1;

    const cJSON *list = cJSON_GetObjectItemCaseSensitive(root, "names");
    if (!cJSON_IsArray(list)) {
        cJSON_Delete(root);
        return -1;
    }
    ...
    const cJSON *item;
    cJSON_ArrayForEach(item, list) {
        if (n >= ST_MAX_SPEAKERS)
            break;
        if (cJSON_IsString(item) && item->valuestring[0])
            copy_str(st->speaking[n++], ST_AUTHOR_LEN, item->valuestring);
    }
    st->speaking_count = n;
    cJSON_Delete(root);
    return 0;
...
int state_is_speaking(const app_state *st, const char *name)
{
    for (int i = 0; i < st->speaking_count; i++) {
        if (strcmp(st->speaking[i], name) == 0)
            return 1;
    }
    return 0;
}
...
static int prepend_chunk(app_state *st, const cJSON *list, int trimmed)
{
    ...
    st->message_count += n;
    /* A short chunk normally means the channel has no more history, but
       the companion also trims a batch that would not fit one frame and
       flags it: treating that as the end would stop scroll-back dead in
       channels of long or non-Latin messages. */
    if (total < ST_HISTORY_CHUNK && !trimmed)
        st->history_done = 1;
    return 0;
}
...
int state_parse_messages(app_state *st, const char *json)
{
    ...
    if (cJSON_HasObjectItem(root, "before")) {
        int trimmed = cJSON_IsTrue(
            cJSON_GetObjectItemCaseSensitive(root, "trimmed"));
        int r = prepend_chunk(st, list, trimmed);
        cJSON_Delete(root);
        return r;
    }
