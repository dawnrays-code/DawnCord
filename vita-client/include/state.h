...
typedef struct {
    ...
    /* Who is talking right now, pushed by the companion when the set
       changes. Matched by display name against the voice-user rows. */
    char speaking[ST_MAX_SPEAKERS][ST_AUTHOR_LEN];
    int speaking_count;

    /* Local playback silenced from the console (Square). Separate from
       anything Discord knows: it only stops this device playing. */
    int voice_muted;
...
/* VOICE_SPEAKING push: replaces the set of people currently talking. */
int state_parse_speaking(app_state *st, const char *json);

/* Is this display name talking right now? */
