...
/* Silence playback without leaving the channel. Packets keep arriving and
   are discarded, so unmuting resumes on the live conversation rather than
   on a backlog. */
void voice_set_muted(int muted);
int voice_is_muted(void);

/* Receive-side link counters: rx, dropped, mid-stream underruns, resyncs,
   drift trims, reordered-then-recovered, concealed. Reported to the
   companion for its log. */
